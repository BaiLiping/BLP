...
void run_cpu_direct() {
  ...
  initialize_gpio_for_output(gpio_port,test1);
  initialize_gpio_for_output(gpio_port,test2);
  initialize_gpio_for_output(gpio_port,test3);
  // Prepare GPIO
  ...
  //int k=18&23&24&25;
  // Generate squarewave of roughly 40KHz
  for (;;) {
