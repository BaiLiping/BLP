...
LR_A = .01    # learning rate for actor
LR_C = .01    # learning rate for critic
...
class Actor(object):
    def __init__(self, sess, n_features, n_actions, lr=LR_A):
        ...
        with tf.variable_scope('Actor'):
            l1 = tf.layers.dense(inputs=self.state,units=512,activation=tf.nn.relu,kernel_initializer=tf.random_normal_initializer(0., .1),bias_initializer=tf.constant_initializer(0.1),name='l1')
            l2=tf.layers.dense(inputs=l1,units=128,activation=tf.nn.relu,kernel_initializer=tf.random_normal_initializer(0,.1),name='l2')
            l3=tf.layers.dense(inputs=l2,units=64,activation=tf.nn.relu,kernel_initializer=tf.random_normal_initializer(0,0.1),name='l3')
            self.action_probability = tf.layers.dense(inputs=l3,units=n_actions,activation=tf.nn.softmax,kernel_initializer=tf.random_normal_initializer(0, .1),bias_initializer=tf.constant_initializer(0.1),name='action_probbability')
...
class Critic(object):
    def __init__(self, sess, n_features, lr=LR_C):
        ...
        with tf.variable_scope('Critic'):
            ...
            l2=tf.layers.dense(inputs=l1,units=128,activation=tf.nn.relu,kernel_initializer=tf.random_normal_initializer(0,.1),name='l2')
            l3=tf.layers.dense(inputs=l2,units=64,activation=tf.nn.relu,kernel_initializer=tf.random_normal_initializer(0,0.1),name='l3')
            self.v = tf.layers.dense(inputs=l3,units=1,activation=None,kernel_initializer=tf.random_normal_initializer(0., .1),bias_initializer=tf.constant_initializer(0.1),name='V')
